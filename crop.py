...
def crop_image_uniform(src_dir, dst_dir):
    f = open("labels/210-uniform.txt", "r")
    if not os.path.exists(dst_dir):
        os.makedirs(dst_dir)
    for page in range(1,4):
        #img = Image.open( src_dir + "/" + str(page) +"-uniform.png").convert('L')
        img = Image.open( src_dir + "/" +"hhstyle.png").convert('L')
        
        width, height = img.size
        cell_width = width/float(cols)
        cell_height = height/float(rows)
        header_offset = height/float(rows) * header_ratio*1.1
        width_margin = cell_width * 0.1
        height_margin = cell_height * 0.01
